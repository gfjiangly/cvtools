...
__device__ inline void poly2float2(float const * const poly, float2 * ps) {
    ps[0].x = poly[0];
    ps[1].x = poly[2];
    ps[2].x = poly[4];
    ps[3].x = poly[6];
...
__device__ inline float devPolyIoU8(float const * const poly1, float const * const poly2) {

    float2 ps1[maxn], ps2[maxn];
    int n1 = 4;
    int n2 = 4;

    poly2float2(poly1, ps1);
    poly2float2(poly2, ps2);
...
// 有bug，待调试
__global__ void overlaps8_kernel(const int N, const int K, const float* dev_boxes,
    const float * dev_query_boxes, float* dev_overlaps) {
    ...
    if ((x < N) && (y < K)) {
        int offset = x * K + y;
        dev_overlaps[offset] = devPolyIoU8(dev_boxes + x * 8, dev_query_boxes + y * 8);
    }
}
...
void _overlaps8(float* overlaps, const float* boxes, const float* query_boxes, int n, int k, int device_id) {
    ...
    CUDA_CHECK(cudaMalloc(&boxes_dev, n * 8 * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(boxes_dev, boxes, n * 8 * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMalloc(&query_boxes_dev, k * 8 * sizeof(float)));
    CUDA_CHECK(cudaMemcpy(query_boxes_dev, query_boxes, k * 8 * sizeof(float), cudaMemcpyHostToDevice));
    ...
    dim3 blocks(DIVUP(n, 32),
                DIVUP(k, 32));
    dim3 threads(32, 32);
    overlaps8_kernel<<<blocks, threads>>>(n, k,
                                          boxes_dev,
                                          query_boxes_dev,
                                          overlaps_dev);
